...
void SunVisualizer::drawGrid() {
    ...
    for (float lat = -80; lat <= 80; lat += 10.0f) {
        for (float lon = -180; lon <= 180; lon += 10.0f) {
            ...
            glVertex3f(x, y, z);
            glVertex3f(x * 1.01f, y * 1.01f, z * 1.01f);
        }
    }
    glEnd();
}
...
void SunVisualizer::drawSunPath() {
    ...
    for (const auto& pos : sunPath) {
        float theta = pos.azimuth * M_PI / 180.0f;
        float phi = pos.altitude * M_PI / 180.0f;
        float r = 1.2f;
...
void SunVisualizer::drawSun(GLfloat* sunLightPosition) {
    ...
    if (sunLightPosition) {
        sunLightPosition[0] = x;
        sunLightPosition[1] = y;
        sunLightPosition[2] = z;
